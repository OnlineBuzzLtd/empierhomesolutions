...
- **E-7 — `is_test` propagation through the webchat → CJ runtime → BookingConfirmed path** (cross-repo). Today's live pane fix widens the filter so webchat bookings show up, but the underlying rows are still `is_test=false` (the public webchat endpoint hardcodes `source=empire_lp` and doesn't accept the flag). End-of-session cleanup therefore can't wipe them — every demo accumulates 1-2 customer rows in prod (verified today: 5+ "shaz"/"zhaz" rows from 2026-05-18 testing). Fix: add a gated `is_test` field to `/api/public/webchat/sessions` (internal-service-token only), CJ runtime persists in conversation state, BookingConfirmed event carries it, command-executor sets `is_test` on resulting Empire CRM rows. Cleanup then catches them via the existing `is_test=true` predicate.
- **E-8 — customer-record-not-updating after subsequent `capture_identity` tool calls** (CJ side). Today's screenshot: webchat customer row created with full_name="zhaz" from the consent form, then the agent captured "Shaz Iqbal" / phone / email / address during the chat, but the Empire customer row never updated — full_name stayed "zhaz", phone + email stayed null. Subsequent `capture_identity` updates aren't reaching `updateCustomerFromPayload`. Investigate in [text-channel-orchestrator.ts](Customer-Journeys-AI-v1/customerjourneys-site/services/platform-api/src/lib/text-channel-orchestrator.ts) / the BookingConfirmed publish path.
