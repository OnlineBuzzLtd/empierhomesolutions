...
- **No rollouts without tests for the thing being rolled out.** Specifically: any new API route, any change to a SQL query or `.from(...)`/`.eq(...)` chain, any new migration, and any change that depends on a column existing — must ship with a test. Typecheck passing is not a test. "I read the code" is not a test. The Demo Console `customer_assets.is_test` bug (2026-05-18) shipped because the cleanup endpoint referenced a column that didn't exist; a single unit test of the endpoint's table list would have blocked the deploy. The minimum bar: a unit test that asserts the API's stated contract against either a mock or a static fixture of the schema. If a real integration test is impractical, write a contract test against an extracted constant (as `tests/unit/demo-console-cleanup-tables.test.ts` does). Skipping this rule for "ship fast" is the exact rationale that produced both Twilio incidents and the cleanup column bug.
