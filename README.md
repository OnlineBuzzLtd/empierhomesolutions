...
- AI quote drafting for safe catalogue-backed jobs: survey-required work can draft after survey completion, fixed-price service bookings can draft after booking, and v1 keeps the quote as a draft rather than auto-sending it
- manager-only record-trail deletion planning/execution for customers, jobs, enquiries, appointments, and AI recovery cases, with operational deletion and financial/audit redaction
...
- `npm run crm:perf:io` — run the Supabase Disk IO audit helper for CRM hot tables and queries
...
- **Scripted webchat autopilot** lives in the operator panel. Pick `AI customer` to let deterministic scenario replies and the linked CustomerJourneys platform AI play the customer against the real CRM webchat agent; pick `Fixed script` when provider fallback is preferred. The three v1 scenarios are emergency repair booking, boiler install survey, and fixed-price service quote.
- **AI quote story controls** can move a demo job through survey done, AI quote draft, and deposit invoice generation. The controls use the same quote-automation module as live CRM rather than a demo-only fake.
...
- **Expandable Live CRM records** show safe linked detail behind customer, enquiry, job, appointment, survey, quote, invoice schedule, invoice, and payment rows so the demo can show the trail behind each live event.
...
- **Cross-repo dependencies**: the platform-api side ([Customer Journeys AI v1](https://github.com/OnlineBuzzLtd/CustomerJourneysAI)) hosts `/v1/internal/crm/tenants/:tenantId/numbers`, `/v1/internal/demo/customer-turn`, and the agent / text-orchestrator. The Demo Console depends on Cloud Run deploys of that repo for the agent's softer wordings, loop-guard exemptions, and platform-owned demo-customer provider calls to be live.
