...
import { EngineerFieldView } from "@/modules/crm/components/jobs/EngineerFieldView";
...
import {
  ...
  jobStatusConfig,
  jobVariationStatusConfig,
  purchaseOrderStatusConfig,
  quoteStatusConfig,
  supplierReconciliationStatusConfig,
...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isEngineer && uiMode === "commusoft") {
    return (
      <CommsoftJobEvent
        job={job}
        notes={notes}
...
async function AdminJobView({
  ...
  notes: Awaited<ReturnType<typeof getJobDetail>> extends { notes: infer N } | null ? N : never;
  expenses: Awaited<ReturnType<typeof getJobDetail>> extends { expenses: infer E } | null ? E : never;
  payments: Awaited<ReturnType<typeof getJobDetail>> extends { payments: infer P } | null ? P : never;
  attachments: Awaited<ReturnType<typeof getJobDetail>> extends { attachments: infer A } | null ? A : never;
  quote: Awaited<ReturnType<typeof getJobDetail>> extends { quote: infer Q } | null ? Q : never;
  invoice: Awaited<ReturnType<typeof getJobDetail>> extends { invoice: infer I } | null ? I : never;
  staff: Awaited<ReturnType<typeof listStaffDirectory>>;
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        title={job.title}
        action={<StatusBadge config={jobStatusConfig[job.status]} />}
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-3">
            ...
            <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">
              {job.description || "No job brief or access note provided."}
            </p>
          </div>

          <ApiForm
            endpoint={`/api/crm/jobs/${job.id}`}
            method="PATCH"
          ...
          >
            ...
            <select
              ...
              defaultValue={job.site_contact_id ?? ""}
              className="rounded-lg border border-slate-300 px-3 py-2 text-sm"
            >
            ...
            job.status === "aborted" ? (
              <p className="rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-800">
                To make this job active in the engineer view again, change the status to Booked or In Progress
                and save.
              </p>
            ) : null}
            <input
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">
                Keep the office schedule and field crew aligned from the same job record.
              </p>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <CollapsibleSectionCard title={`Job Phases (${job.phases?.length ?? 0})`} defaultOpen={true}>
              ...
              {job.phases.map((phase) => (
                <li key={phase.id} className="rounded-lg border border-slate-200 p-4">
                  ...
                  <p className="mt-3 text-sm text-slate-600">
                    {phase.description || "No phase detail recorded."}
                  </p>
                  {phase.completed_at ? (
          ...
          <div className="mt-4 border-t border-slate-100 pt-4">
            <ApiForm
              endpoint={`/api/crm/jobs/${job.id}/phases`}
        ...
        <CollapsibleSectionCard title={`Variations (${job.variations?.length ?? 0})`} defaultOpen={true}>
            ...
            <ul className="space-y-3">
              {job.variations.map((variation) => (
                <li key={variation.id} className="rounded-lg border border-slate-200 p-4">
                  ...
                  <p className="mt-3 text-sm text-slate-600">
                    {variation.description || "No variation detail recorded."}
                  </p>
                  {canManageCommercials && variation.status === "draft" ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      <ApiForm
                        endpoint={`/api/crm/jobs/${job.id}/variations/${variation.id}`}
                        method="PATCH"
                        submitLabel="Approve Variation"
                        className="space-y-0"
                      >
          ...
          <div className="mt-4 border-t border-slate-100 pt-4">
            <ApiForm
              endpoint={`/api/crm/jobs/${job.id}/variations`}
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <CollapsibleSectionCard title={`Hazards (${job.hazards?.length ?? 0})`} defaultOpen={true}>
              ...
              {job.hazards.map((hazard) => (
                <li key={hazard.id} className="rounded-lg border border-slate-200 p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    {hazard.description || "No hazard detail recorded."}
                  </p>
          ...
          <div className="mt-4 border-t border-slate-100 pt-4">
            <ApiForm
              endpoint={`/api/crm/jobs/${job.id}/hazards`}
        ...
        <CollapsibleSectionCard title={`Checklists (${job.checklists?.length ?? 0})`} defaultOpen={true}>
          {job.checklists && job.checklists.length > 0 ? (
            <ul className="space-y-3">
              {job.checklists.map((checklist) => (
                <li
                  key={checklist.id}
                  className={`rounded-lg border p-4 ${checklist.is_mandatory && checklist.status !== "completed" ? "border-rose-200 bg-rose-50" : "border-slate-200"}`}
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      {checklist.completed_at ? (
                        <p className="mt-1 text-xs text-slate-500">
                          Completed {formatDateTime(checklist.completed_at)}
                        </p>
                      ) : null}
                    </div>
          ...
          <div className="mt-4 border-t border-slate-100 pt-4">
            <ApiForm
              endpoint={`/api/crm/jobs/${job.id}/checklists`}
            ...
            >
              ...
              {canManageCommercials ? (
                <label className="flex items-center gap-2 text-sm text-slate-700">
                  ...
                  />
                  Mandatory — must be completed before job can be marked done
                </label>
              ) : null}
            </ApiForm>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <CollapsibleSectionCard title={`Certificates (${job.certificates?.length ?? 0})`} defaultOpen={true}>
                ...
                <li key={certificate.id} className="rounded-lg border border-slate-200 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        {certificate.certificate_number || "No certificate number"} · Issued{" "}
                  ...
                  {certificate.status !== "completed" && certificate.status !== "sent" ? (
                    <div className="mt-3">
                      <ApiForm
                        endpoint={`/api/crm/jobs/${job.id}/certificates/${certificate.id}`}
                        method="PATCH"
          ...
          <div className="mt-4 border-t border-slate-100 pt-4">
            <ApiForm
              endpoint={`/api/crm/jobs/${job.id}/certificates`}
        ...
        <SectionCard
          title={`Supplier Control (${(job.purchaseOrders?.length ?? 0) + (job.supplierReconciliation?.length ?? 0)})`}
        >
          <div className="space-y-4">
            <div>
                    ...
                    <li key={po.id} className="rounded-lg border border-slate-200 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">
                            {po.supplier?.name || "No supplier"} · {formatCurrency(po.total_amount)}
                          </p>
                        </div>
              ...
              <div className="mt-3">
                <ApiForm
                  endpoint={`/api/crm/jobs/${job.id}/purchase-orders`}
            ...
            <div className="border-t border-slate-100 pt-4">
                    ...
                    <li key={entry.id} className="rounded-lg border border-slate-200 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">
                            {entry.entry_type} · {entry.supplier?.name || "No supplier"} ·{" "}
              ...
              <div className="mt-3">
                <ApiForm
                  endpoint={`/api/crm/jobs/${job.id}/supplier-reconciliation`}
                  submitLabel="Add Supplier Entry"
                  className="grid gap-3 md:grid-cols-2"
                >
                  <select name="supplier_id" className="rounded-lg border border-slate-300 px-3 py-2 text-sm">
                    <option value="">Select supplier…</option>
      ...
      {/* Expenses + Payments */}
      <div className="grid gap-6 xl:grid-cols-2">
        <CollapsibleSectionCard title={`Expenses (${expenses.length})`} defaultOpen={true}>
          {expenses.length === 0 ? <EmptyState message="No expenses logged." /> : null}
          <ul className="space-y-2">
            {expenses.map((expense) => (
              <li
                key={expense.id}
                className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-3 text-sm"
              >
                <div>
